...
const routes: Record<string, unknown> = {
  ...
  '/api/history': [{ id: 'w2', winnerId: 'p2', previousKingId: 'p1', runnerUpId: 'p1', participantIds: ['p1', 'p2'] }, { id: 'w1', winnerId: 'p1', previousKingId: null }],
};
...
describe('MCP-servern', () => {
  ...
  it('bordet och finalförloraren skickas som id', async () => {
    await callTool('pingis_record_win', { winner: 'Anna', runnerUp: 'erik', participants: ['Erik', 'anna'] });
    expect(postedWins.at(-1)).toEqual({ winnerId: 'p2', runnerUpId: 'p1', participantIds: ['p1', 'p2'] });
  });

  it('deltagare som inte är en lista kröner ingen, i stället för att tyst tappas', async () => {
    const before = postedWins.length;
    const result = await callTool('pingis_record_win', { winner: 'Anna', participants: 'Anna, Erik' });
    expect(result.isError).toBe(true);
  ...
  it('historiken namnger vinnaren och den avsatte', async () => {
    expect(await toolJson('pingis_history', { limit: 1 })).toEqual([expect.objectContaining({ winner: 'Anna', previousKing: 'Erik', runnerUp: 'Erik', participants: ['Erik', 'Anna'] })]);
  });
